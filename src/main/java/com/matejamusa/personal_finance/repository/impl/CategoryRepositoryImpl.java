...
public class CategoryRepositoryImpl implements CategoryRepository<Category> {
    ...
    public Category createCategoryForUser(Category category, Long userId) {
        ...
        Integer count = jdbc.queryForObject(COUNT_CATEGORY_BY_NAME_AND_TYPE, Map.of("name", category.getName(), "type", category.getType()), Integer.class);
        if (count == null || count == 0) {
            newCategory = createCategory(category);
        } else {
            newCategory = jdbc.queryForObject(SELECT_CATEGORY_BY_NAME_AND_TYPE, Map.of("name", category.getName(), "type", category.getType()), new CategoryRowMapper());
        }

        System.out.println(newCategory.toString());
        jdbc.update(CREATE_USER_CATEGORY, Map.of("userId", userId, "categoryId", newCategory.getId()));
        return newCategory;
    }

    private Category createCategory(Category category) {
        KeyHolder holder = new GeneratedKeyHolder();
        jdbc.update(CREATE_CATEGORY,getSqlParameterSource(category), holder);
